...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-50 flex h-full w-64 flex-col bg-white/95 backdrop-blur-xl transition-transform duration-300 lg:static lg:translate-x-0 ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center gap-3 border-b border-slate-200/60 px-5 py-5">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-primary-600 to-violet-600 shadow-lg shadow-primary-600/20">
        ...
        <div className="border-t border-slate-200/60 px-4 py-4">
          <p className="text-xs text-slate-400">Bright Future v1.0</p>
        </div>
      </aside>
    </>
  )
